...
public class SseChatController {
    ...
    // produces : 서버측에서 이러한 데이터를 생산함.
    // 일반적인 코드를 작성할때 기본적으로 다생략했음
    // produces = MediaType.TEXT_HTML_VALUE - HTML 파일 형식 (뷰 리졸브)
    // produces = MediaType.APPLICATION_JSON_VALUE
    // HTTP/1.1 OK
    // Content-Type: text/event-stream
    // 2. 클라이언트가 이 주소를 구독함 (sse 구독 이벤트 처리)
    @GetMapping(value = "/sse/connect", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    @ResponseBody // 뷰가 아니라 객체(Emitter)를 반환해야함
    public SseEmitter connect() {

        // clientId는 서버측에서만 관리하는 값이다. (js 측은 전혀 몰라도됨)
    ...
    public String sendMessage(@RequestParam(name = "sender") String sender,
        ...
        sseChatService.addMessage(sender, message);
