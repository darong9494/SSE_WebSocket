...
public class SseChatService {
    ...
    // 접속한 클라이언트들을 보관할 자료구조 >> ConcurrentHashMap 멀티 스레드에서 안전하다.
    private final Map<String, SseEmitter> emitterMap = new ConcurrentHashMap<>();

    // 1. 클라이언트가 접속하면 호출됨 (SSE연결)
    public SseEmitter createConnection(String clientId) {
        ...
        // 필수 콜백 함수등록 처리
        // 연결 종료/ 타임아웃시 Map 목록에서 제거처리 (불필요한 메모리 누수 방지)
        ...
        // 참고 : 0L로 생성하면 무한으로 계속 연결이 되지만 실무에서는 좀비 커넥션이 생성됨
        // 실제로 0L로 하지않음 - 폴링방식 아님
        try {
    ...
    private void broadcast(String message, String sender) {
        ...
        String formattedMessage = sender + ": " + message; // 반드시 공백한칸
